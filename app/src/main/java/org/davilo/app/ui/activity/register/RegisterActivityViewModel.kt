...
import org.davilo.app.model.LoginFormState
import org.davilo.app.model.LoginOutput

class RegisterActivityViewModel @ViewModelInject constructor(
    ...
    private fun onFailure(action: Throwable?) {
        if (action is ServerException) {
            _loginForm.value =
                LoginFormState(passwordErrorString = action.output?.status?.status_message)
        }
    }
    ...
    fun invalidateLoginFormState() {
        if (appPreferences.getToken() != null) {
            _loginForm.value =
                LoginFormState(isSignedIn = true)
